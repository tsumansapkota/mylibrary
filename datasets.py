...
#         for name in filename:
#             print("Downloading " + name[1] + "...")
#             request.urlretrieve(os.path.join(base_url, name[1]), os.path.join(dirs, name[1]))
#         print("Download complete.")
...
#         mnist = {}
#         for name in filename[:2]:
#             with gzip.open(os.path.join(dirs, name[1]), 'rb') as f:
#                 mnist[name[0]] = np.frombuffer(f.read(), np.uint8, offset=16).reshape(-1, 28 * 28)
#         for name in filename[-2:]:
#             with gzip.open(os.path.join(dirs, name[1]), 'rb') as f:
#                 mnist[name[0]] = np.frombuffer(f.read(), np.uint8, offset=8)

#         with open(os.path.join(dirs, "fmnist.pkl"), 'wb') as f:
#             pickle.dump(mnist, f)
#         print("Save complete.")
...
#         # lib_dir = os.path.dirname(os.path.realpath(__file__))# + dirs
#         # dirs = os.path.join(lib_dir,dirs) + '/'
#         if not os.path.exists(os.path.join(dirs, "fmnist.pkl")):
#             self.download_mnist(dirs)
#             self.save_mnist(dirs)
#         with open(os.path.join(dirs, "fmnist.pkl"), 'rb') as f:
#             mnist = pickle.load(f)
#         return mnist["training_images"], mnist["training_labels"], mnist["test_images"], mnist["test_labels"]
