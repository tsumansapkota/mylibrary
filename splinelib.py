...
####################################################################################
 ...
 ##################################################################################   
    ...
    def _maintain_good_spline_(self):
        self._remove_close_points_()
        self._combine_linear_points_()
        self._remove_no_input_points_()
        self._add_new_point_()
...
class SplineVectorLayer(object):
    ...
    def __init__(self, input_dim, max_points, epsilon=0.1):
        ...
        for _ in range(input_dim):
            self.spline_list.append(Spline1D(max_points, x=np.array([-1,1]), y=np.array([-1,1])))
        self.input = None
        self.output = None
        self.del_output = None
